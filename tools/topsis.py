# Taken from https://github.com/Glitchfix/TOPSIS-Python, slightly modified
...
class Topsis():
    ...
    def __init__(self, evaluation_matrix, weight_matrix, criteria, debug=False):
        # M×N matrix
    ...
    def calc(self):
        if self.debug:
            print("Step 1 - Evaluation Matrix:\n", self.evaluation_matrix, end="\n\n")
        self.step_2()
        if self.debug:
            print("Step 2 - Normalized Evaluation Matrix:\n", self.normalized_decision, end="\n\n")
        self.step_3()
        if self.debug:
            print("Step 3 - Weighted Normalized Evaluation Matrix\n", self.weighted_normalized, end="\n\n")
        self.step_4()
        if self.debug:
            print("Step 4 - worst_alternatives | best_alternatives \n", self.worst_alternatives, " | ", self.best_alternatives, end="\n\n")
        self.step_5()
        if self.debug:
            print("Step 5 - Distances to Worst Alternative | Distances to Best Alternative\n", self.worst_distance, self.best_distance, end="\n\n")
        self.step_6()
        if self.debug:
            print("Step 6 - Similarites to Worst Alternative | Similarities to Best Alternative\n", self.worst_similarity, self.best_similarity, end="\n\n")
